...
_ROUTING_PROMPT_TEXT = """You are a routing supervisor for a prior authorization services team.
        ...
        try:
            ...
            if getattr(_s, "LANGFUSE_PROMPT_VERSIONING_ENABLED", False):
                ...
                self.system_prompt = _tracer.get_prompt_or_default(
                    "csip-pa-routing-prompt", _ROUTING_PROMPT_TEXT, label=_label
                )
                self.planning_prompt = _tracer.get_prompt_or_default(
                    "csip-pa-planning-prompt", _PLANNING_PROMPT_TEXT, label=_label
                )
        except Exception:
