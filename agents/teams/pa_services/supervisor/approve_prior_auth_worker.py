"""
PA Services: Approve Prior Authorization Worker Agent
"""
...
WORKER_PROMPT = (
            "You are a prior authorization approval specialist for a health insurance company. "
            "Your role is to approve a prior authorization request. "
            "You MUST call the approve_prior_auth tool to perform the approval — never answer from memory or context. "
            "You must extract from the query: the PA ID and the clinical justification reason for approval. "
...
class ApprovePriorAuthWorker:
    """Worker agent for prior authorization approval operations."""

    def __init__(self):
        self.name = "approve_prior_auth_worker"

        mcp_client = PAServicesMCPToolClient()
        tool = mcp_client.get_tool("approve_prior_auth")
        if tool is None:
            raise RuntimeError("approve_prior_auth not found in PAServicesMCPToolClient")
        self.tool = tool
        ...
        try:
            ...
            if getattr(_s, "LANGFUSE_PROMPT_VERSIONING_ENABLED", False):
                ...
                _prompt = _tracer.get_prompt_or_default(
                    "csip-pa-approve-worker-prompt", WORKER_PROMPT, label=_label
                )
        except Exception:
    ...
    def execute(self, query: str, user_id: str, user_role: str, session_id: str, execution_id: str = "") -> Dict[str, Any]:
        """Execute ApprovePriorAuthWorker task with error handling and retry logic."""
