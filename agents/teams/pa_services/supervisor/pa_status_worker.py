"""
PA Services: PA Status Worker Agent
"""
...
class PAStatusWorker:
    ...
    def __init__(self):
        self.name = "pa_status_worker"

        mcp_client = PAServicesMCPToolClient()
        tool = mcp_client.get_tool("pa_status")
        if tool is None:
            raise RuntimeError("pa_status not found in PAServicesMCPToolClient")
        self.tool = tool
    ...
    def execute(self, query: str, user_id: str, user_role: str, session_id: str) -> Dict[str, Any]:
        ...
        while retry_count <= max_retries:
            try:
                ...
                # Audit action — pa_status is keyed by pa_id, not pa_number
