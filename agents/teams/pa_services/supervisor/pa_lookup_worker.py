"""
PA Services: PA Lookup Worker Agent
"""
...
WORKER_PROMPT = (
            "You are a prior authorization information specialist for a health insurance company. "
            "Your role is to look up detailed prior authorization information by PA ID. "
            "You MUST call the pa_lookup tool to answer — never answer from memory or context. "
            "Look up the PA using the PA ID provided in the query. "
...
class PALookupWorker:
    """Worker agent for prior authorization lookup operations."""

    def __init__(self):
        self.name = "pa_lookup_worker"

        mcp_client = PAServicesMCPToolClient()
        tool = mcp_client.get_tool("pa_lookup")
        if tool is None:
            raise RuntimeError("pa_lookup not found in PAServicesMCPToolClient")
        self.tool = tool
        ...
        try:
            ...
            if getattr(_s, "LANGFUSE_PROMPT_VERSIONING_ENABLED", False):
                ...
                _prompt = _tracer.get_prompt_or_default(
                    "csip-pa-lookup-worker-prompt", WORKER_PROMPT, label=_label
                )
        except Exception:
    ...
    def execute(self, query: str, user_id: str, user_role: str, session_id: str, execution_id: str = "") -> Dict[str, Any]:
        """Execute PALookupWorker task with error handling and retry logic."""
