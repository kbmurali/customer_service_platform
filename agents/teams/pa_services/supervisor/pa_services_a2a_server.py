"""
PA Services Remote Agent Server (A2A Protocol)
=========================================================
Receives A2A task envelopes from A2AClientNode (central supervisor),
decrypts, executes the PA services supervisor's LangGraph subgraph,
...
async def lifespan(app: FastAPI):
    ...
    asyncio.run() inside PAServicesMCPToolClient.__init__ runs
    ...
    _get_graph()        # triggers PAServicesSupervisor.__init__ → workers → MCP client
...
# ---------------------------------------------------------------------------
app = FastAPI(
    title="PA Services A2A Agent",
    version="1.0.0",
    description="Remote A2A agent for prior authorization services with encrypted communication",
    lifespan=lifespan,
)
...
SUPERVISOR_NAME = "pa_services_supervisor"
...
def _get_secure_bus():
    ...
    if _secure_bus is None:
        _secure_bus = get_secure_message_bus(schema_registry=build_pa_schema_registry())

    return _secure_bus


def _get_graph() -> CompiledStateGraph:
    """Return (and cache) the compiled PA services LangGraph."""
    global _graph

    if _graph is None:
        _graph = get_pa_services_graph()
...
def _get_agent_card():
    ...
    if _agent_card is None:
        base_url = os.getenv("A2A_PA_SERVICES_URL", "https://api-gateway:8443/a2a/pa")
        _agent_card = build_pa_services_agent_card(base_url)
...
async def a2a_tasks_send(request: Request):
    """
    PA Services A2A task endpoint: receive an encrypted A2A task envelope,
    ...
    try:
        ...
        logger.info(
            "PA Services A2A task %s received from %s",
            task_id,
        ...
        logger.info(
            "PA Services A2A task %s: user=%s session=%s query_len=%d",
            task_id,
            extracted["user_id"],
        ...
        # ── Step 4: Execute the PA services subgraph ──
        ...
        logger.info(
            "PA Services A2A task %s completed in %.0f ms (state=%s)",
            task_id,
            elapsed_ms,
            a2a_state,
        )
        ...
        if isinstance(exc, SecurityError):
            logger.error("Security error on PA Services A2A task %s: %s", task_id_err, exc)
        ...
        logger.error("Execution error on PA Services A2A task %s: %s", task_id_err, exc, exc_info=True)
