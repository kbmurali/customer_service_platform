"""
Claims Services: Claim Payment Info Worker Agent
"""
...
class ClaimPaymentInfoWorker:
    """Worker agent for claim payment information operations."""

    def __init__(self):
        self.name = "claim_payment_info_worker"

        mcp_client = ClaimServicesMCPToolClient()
        tool = mcp_client.get_tool("claim_payment_info")
        if tool is None:
            raise RuntimeError("claim_payment_info not found in ClaimServicesMCPToolClient")
        self.tool = tool
        ...
        try:
            ...
            if getattr(_s, "LANGFUSE_PROMPT_VERSIONING_ENABLED", False):
                ...
                _prompt = _tracer.get_prompt_or_default(
                    "csip-claims-payment-worker-prompt", WORKER_PROMPT, label=_label
                )
        except Exception:
    ...
    def execute(self, query: str, user_id: str, user_role: str, session_id: str, execution_id: str = "") -> Dict[str, Any]:
        """Execute ClaimPaymentInfoWorker task with error handling and retry logic."""
