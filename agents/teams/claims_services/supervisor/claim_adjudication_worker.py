...
rules to produce a structured recommendation with justification.
...
WORKER_PROMPT = (
    "You are a claims adjudication specialist for a health insurance company.\n"
    "You receive evidence gathered from prior steps about a specific claim and "
    "must evaluate whether the claim should be approved, denied, or sent for review.\n\n"
    "EVIDENCE AVAILABLE IN THE QUERY:\n"
    "The query contains results from prior plan steps including:\n"
    "- claim_lookup: claim details (procedure, dates, amounts, status)\n"
    "- check_eligibility: member eligibility on the service date\n"
    "- provider_network_check: whether the provider is in-network\n"
    "- pa_lookup or pa_requirements: prior authorization status if applicable\n\n"
    "ADJUDICATION RULES:\n"
    "1. Member must be ACTIVE and eligible on the service date.\n"
    "2. Provider should be in-network; out-of-network claims require REVIEW.\n"
    "3. If the procedure requires prior authorization, a PA must exist with status APPROVED.\n"
    "4. If required PA is missing or DENIED, recommend DENY with specific reason.\n"
    "5. If all coverage criteria are met, recommend APPROVE.\n"
    "6. If information is insufficient to make a determination, recommend REVIEW.\n\n"
    "You MUST respond with ONLY a JSON object (no markdown, no explanation outside JSON):\n"
    "{\n"
    '    "recommendation": "APPROVE" or "DENY" or "REVIEW",\n'
    '    "justification_summary": "One paragraph explaining the decision with specific evidence cited",\n'
    '    "evidence_used": ["eligibility:ACTIVE", "network:IN", "pa_required:YES", "pa_found:APPROVED"]\n'
    "}\n\n"
    "The evidence_used array must contain short key:value pairs summarizing each "
    "piece of evidence you considered. Use the exact format shown above."
)


class ClaimAdjudicationWorker:
    """Decision worker for claims adjudication.
    ...
    def __init__(self):
        self.name = "claim_adjudication_worker"
        self.tool_name = "claim_adjudication"  # for audit logging
        ...
        try:
            ...
            if getattr(_s, "LANGFUSE_PROMPT_VERSIONING_ENABLED", False):
                ...
                self.system_prompt = _tracer.get_prompt_or_default(
                    "csip-claims-adjudication-worker-prompt", WORKER_PROMPT, label=_label
                )
        except Exception:
    ...
    def execute(self, query: str, user_id: str, user_role: str, session_id: str, execution_id: str = "") -> Dict[str, Any]:
        """Execute claim adjudication with error handling and retry logic."""
        ...
        while retry_count <= max_retries:
            try:
                ...
                try:
                    ...
                    if recommendation not in ("APPROVE", "DENY", "REVIEW"):
                        recommendation = "REVIEW"
                except (json.JSONDecodeError, AttributeError) as parse_err:
                    logger.warning(
                        "%s: failed to parse LLM JSON, defaulting to REVIEW: %s",
                        self.name, parse_err,
                    )
                ...
                decision_output = (
                    f"CLAIM ADJUDICATION RECOMMENDATION: {recommendation}\n\n"
