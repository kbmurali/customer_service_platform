"""
Claims Services: Update Claim Status Worker Agent
"""
...
WORKER_PROMPT = (
            "You are a claims processing specialist for a health insurance company. "
            "Your role is to update the status of a claim. "
            "You MUST call the update_claim_status tool to perform the update — never answer from memory or context. "
            "You must extract from the query: the claim ID, the new status, and the reason for the change. "
            "Valid target statuses are: SUBMITTED, UNDER_REVIEW, APPROVED, DENIED. "
...
class UpdateClaimStatusWorker:
    """Worker agent for claim status update operations."""

    def __init__(self):
        self.name = "update_claim_status_worker"

        mcp_client = ClaimServicesMCPToolClient()
        tool = mcp_client.get_tool("update_claim_status")
        if tool is None:
            raise RuntimeError("update_claim_status not found in ClaimServicesMCPToolClient")
        self.tool = tool
        ...
        try:
            ...
            if getattr(_s, "LANGFUSE_PROMPT_VERSIONING_ENABLED", False):
                ...
                _prompt = _tracer.get_prompt_or_default(
                    "csip-claims-update-status-worker-prompt", WORKER_PROMPT, label=_label
                )
        except Exception:
    ...
    def execute(self, query: str, user_id: str, user_role: str, session_id: str, execution_id: str = "") -> Dict[str, Any]:
        """Execute UpdateClaimStatusWorker task with error handling and retry logic."""
        ...
        while retry_count <= max_retries:
            try:
                ...
                # (e.g. claim_id, new_status) that may be redacted from the scrubbed output.
                _raw_tool_output = ""
