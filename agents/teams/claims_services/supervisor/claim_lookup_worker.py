"""
Claims Services: Claim Lookup Worker Agent
"""
...
WORKER_PROMPT = (
            ...
            "Your role is to look up detailed claim information by claim ID. "
            "You MUST call the claim_lookup tool to answer — never answer from memory or context. "
            "Look up the claim using the claim ID provided in the query. "
...
class ClaimLookupWorker:
    """Worker agent for claim lookup operations."""

    def __init__(self):
        self.name = "claim_lookup_worker"

        mcp_client = ClaimServicesMCPToolClient()
        tool = mcp_client.get_tool("claim_lookup")
        if tool is None:
            raise RuntimeError("claim_lookup not found in ClaimServicesMCPToolClient")
        self.tool = tool
        ...
        try:
            ...
            if getattr(_s, "LANGFUSE_PROMPT_VERSIONING_ENABLED", False):
                ...
                _prompt = _tracer.get_prompt_or_default(
                    "csip-claims-lookup-worker-prompt", WORKER_PROMPT, label=_label
                )
        except Exception:
    ...
    def execute(self, query: str, user_id: str, user_role: str, session_id: str, execution_id: str = "") -> Dict[str, Any]:
        """Execute ClaimLookupWorker task with error handling and retry logic."""
