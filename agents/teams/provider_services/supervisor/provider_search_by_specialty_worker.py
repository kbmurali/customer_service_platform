"""
Provider Services: Provider Search by Specialty Worker Agent
"""
...
class ProviderSearchBySpecialtyWorker:
    ...
    def __init__(self):
        self.name = "provider_search_by_specialty_worker"

        mcp_client = ProviderServicesMCPToolClient()
        tool = mcp_client.get_tool("provider_search_by_specialty")
        if tool is None:
            raise RuntimeError("provider_search_by_specialty not found in ProviderServicesMCPToolClient")
        self.tool = tool
    ...
    def execute(self, query: str, user_id: str, user_role: str, session_id: str, execution_id: str = "") -> Dict[str, Any]:
        ...
        while retry_count <= max_retries:
            try:
                ...
                # Raw fields: count (int), providers (array of provider objects)
                _raw_tool_output = ""
