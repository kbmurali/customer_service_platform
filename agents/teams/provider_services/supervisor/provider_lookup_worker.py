"""
Provider Services: Provider Lookup Worker Agent
"""
...
WORKER_PROMPT = (
                    "You are a provider information specialist for a health insurance company. "
                    "You MUST call the provider_lookup tool to answer — never answer from memory or context. "
                    "Look up provider information by provider ID. "
                    "Provider records include providerId, npi, providerType, specialty, phone, "
                    "street, city, state, zipCode, and organizationName or firstName/lastName "
                    "depending on whether the provider is an ORGANIZATION or INDIVIDUAL. "
...
class ProviderLookupWorker:
    """Worker agent for provider lookup operations."""

    def __init__(self):
        self.name = "provider_lookup_worker"

        mcp_client = ProviderServicesMCPToolClient()
        tool = mcp_client.get_tool("provider_lookup")
        if tool is None:
            raise RuntimeError("provider_lookup not found in ProviderServicesMCPToolClient")
        self.tool = tool
        ...
        try:
            ...
            if getattr(_s, "LANGFUSE_PROMPT_VERSIONING_ENABLED", False):
                ...
                _prompt = _tracer.get_prompt_or_default(
                    "csip-provider-lookup-worker-prompt", WORKER_PROMPT, label=_label
                )
        except Exception:
    ...
    def execute(self, query: str, user_id: str, user_role: str, session_id: str, execution_id: str = "") -> Dict[str, Any]:
        """Execute ProviderLookupWorker task with error handling and retry logic."""
