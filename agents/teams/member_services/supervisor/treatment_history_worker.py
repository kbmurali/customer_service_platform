"""
Member Services: Treatment History Worker Agent
(Decision agent enhancements — M6)
"""
...
WORKER_PROMPT = (
    "You are a treatment history specialist for a health insurance company. "
    "You MUST call the treatment_history tool to answer — never answer from memory or context. "
    "Retrieve treatment history for a member by member ID. "
    "You may optionally filter by treatment_type (e.g. PHYSICAL_THERAPY, MEDICATION_TRIAL, "
    "INJECTION, IMAGING, WEIGHT_COUNSELING) or by procedure_code (CPT code). "
...
class TreatmentHistoryWorker:
    """Worker agent for retrieving member treatment history."""

    def __init__(self):
        self.name = "treatment_history_worker"

        mcp_client = MemberServicesMCPToolClient()
        tool = mcp_client.get_tool("treatment_history")
        if tool is None:
            raise RuntimeError("treatment_history not found in MemberServicesMCPToolClient")
        self.tool = tool
        ...
        try:
            ...
            if getattr(_s, "LANGFUSE_PROMPT_VERSIONING_ENABLED", False):
                ...
                _prompt = _tracer.get_prompt_or_default(
                    "csip-member-treatment-history-worker-prompt", WORKER_PROMPT, label=_label
                )
        except Exception:
    ...
    def execute(self, query: str, user_id: str, user_role: str, session_id: str, execution_id: str = "") -> Dict[str, Any]:
        """Execute TreatmentHistoryWorker task with error handling and retry logic."""
        ...
        while retry_count <= max_retries:
            try:
                ...
                # Check ToolMessages for structured error JSON
