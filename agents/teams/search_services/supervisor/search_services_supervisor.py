...
_ROUTING_PROMPT_TEXT = """You are a routing supervisor for a search services team.
...
Available workers:
- search_knowledge_base: Semantic search over FAQs, clinical guidelines, and regulations — requires a query and source (faqs, guidelines, regulations, or all)
- search_medical_codes: Semantic search over CPT procedure codes and ICD-10 diagnosis codes — requires a query and code_type (procedure, diagnosis, or both)
- search_policy_info: Semantic search over policy documents — requires a query and plan_type (HMO, PPO, EPO, or POS)
...
2. If the step cannot be completed because required information is missing
   (no query, no source, no code_type, no plan_type), respond with "SKIP".
3. NEVER respond with FINISH, CONTINUE, or any value not in the worker list.
4. Only use exact worker names: search_knowledge_base, search_medical_codes, search_policy_info.
...
Routing Hints (metadata only — use to select correct filter values, not to answer the query):
{semantic_context}

Available workers (use EXACT names only):
- search_knowledge_base: Semantic search over FAQs, clinical guidelines, and regulations —
  requires a natural-language query and source filter (faqs, guidelines, regulations, or all)
- search_medical_codes: Semantic search over CPT procedure codes and ICD-10 diagnosis codes —
  requires a natural-language description and code_type (procedure, diagnosis, or both)
- search_policy_info: Semantic search over policy documents for plan details, premiums,
  deductibles, and out-of-pocket maximums — requires a natural-language query and plan_type
  (HMO, PPO, EPO, or POS)
...
4. If the query requires both knowledge base content and policy information, make knowledge
   base search the first step so its results can enrich subsequent steps.
...
7. Note: search_knowledge_base uses source; search_medical_codes uses code_type;
   search_policy_info uses plan_type. Include the correct filter parameter in the
   step action so the worker knows which to use.
8. GOAL DECOMPOSITION — create a separate goal for each distinct user intent.
   ...
   MUST be separate goals:
     - "What does my HMO policy say about deductibles?" AND "Find the CPT code for knee surgery?" → 2 goals
     - "Search FAQs about referrals" AND "Look up PPO policy premiums" → 2 goals
   A single goal is only correct when all steps serve one unified intent, e.g.:
     - "Find the CPT code and check if it is covered under my PPO" → 1 goal, 2 steps (same subject)
9. search_policy_info requires a plan_type — do not add a search_knowledge_base step before
   search_policy_info unless broader context (FAQs, guidelines) was explicitly requested.
...
{{
    ...
    "steps": [
        {{
            "step_id": "step_1",
            "goal_id": "goal_1",
            "action": "Search knowledge base for referral FAQs",
            "worker": "search_knowledge_base"
        }}
    ]
}}"""


class SearchServicesSupervisor:
    """
    LangGraph-based supervisor for Search Services team.
    Routes queries to appropriate workers using LangGraph state machine.
    """

    def __init__(self):
        self.name = "search_services_supervisor"
        self.workers = {
            "search_knowledge_base": SearchKnowledgeBaseWorker(),
            "search_medical_codes":  SearchMedicalCodesWorker(),
            "search_policy_info":    SearchPolicyInfoWorker(),
        }
        ...
        try:
            ...
            if getattr(_s, "LANGFUSE_PROMPT_VERSIONING_ENABLED", False):
                ...
                self.system_prompt = _tracer.get_prompt_or_default(
                    "csip-search-routing-prompt", _ROUTING_PROMPT_TEXT, label=_label
                )
                self.planning_prompt = _tracer.get_prompt_or_default(
                    "csip-search-planning-prompt", _PLANNING_PROMPT_TEXT, label=_label
                )
        except Exception:
    ...
    def create_plan_node(self, state: SupervisorState) -> SupervisorState:
        ...
        try:
            ...
            # Extract lightweight metadata hints from Chroma to inform filter
            # selection only (plan_type, source). Document content is intentionally
            # excluded — full retrieval is the responsibility of the search workers,
            # not the planner. Providing content here would pre-answer the query and
            # make the worker steps redundant.
            ...
            try:
                ...
                semantic_context_json = {
                    'detected_plan_types': list({
                        r['metadata'].get('plan_type')
                        for r in policy_context
                        if r.get('metadata', {}).get('plan_type')
                    }),
                    'detected_sources': list({
                        r['metadata'].get('source')
                        for r in faq_context
            ...
            semantic_context = json.dumps(semantic_context_json, indent=2) if semantic_context_json else 'No filter hints available.'
    ...
    def supervisor_node(self, state: SupervisorState) -> SupervisorState:
        ...
        """
        VALID_WORKERS = {"search_knowledge_base", "search_medical_codes", "search_policy_info"}
        ...
        try:
            ...
            if approval_wf.is_circuit_breaker_active():
                ...
                self.audit.log_action(
                    ...
                    resource_type="SEARCH_SERVICES_AGENT",
                    resource_id=""
                )
                return {
                    "next": "FINISH",
        ...
        # or plan type extracted from a prior search result).
        ...
        self.audit.log_action(
            user_id=user_id,
            action="search_services_routing",
            resource_type="SEARCH_SERVICES_AGENT",
            resource_id="",
        )
    ...
    def worker_node(self, worker_name: str):
        ...
        def node(state: SupervisorState) -> SupervisorState:
            ...
            # subsequent search_policy_info or search_medical_codes step).
            # tool_raw_output is the unredacted structured JSON from the MCP server —
            # preferred over 'output' which may have PII scrubbed.
            tool_results = state.get("tool_results", {})
            if tool_results:
    ...
    def create_graph(self) -> CompiledStateGraph:
        ...
        Flow:
            create_plan
                → supervisor
                    → search_knowledge_base → goal_advance → supervisor
                    → search_medical_codes  → goal_advance → supervisor
                    → search_policy_info    → goal_advance → supervisor
                    → error_handler → END
                    → END  (when FINISH)
        """
        ...
        ) -> Literal["search_knowledge_base", "search_medical_codes", "search_policy_info",
...
@lru_cache
def get_search_services_graph():
    """Get or create Search services LangGraph."""
    search_services_supervisor = SearchServicesSupervisor()

    return search_services_supervisor.create_graph()
