"""
Search Services: Search Medical Codes Worker Agent
"""
...
WORKER_PROMPT = (
            "You are a medical codes search specialist for a health insurance company. "
            "Your role is to perform semantic searches over CPT procedure codes and "
            "ICD-10 diagnosis codes to find relevant medical codes. "
            "You MUST call the search_medical_codes tool to answer — never answer from memory or training data. "
...
class SearchMedicalCodesWorker:
    """Worker agent for semantic medical code search operations."""

    def __init__(self):
        self.name = "search_medical_codes_worker"

        mcp_client = SearchServicesMCPToolClient()
        tool = mcp_client.get_tool("search_medical_codes")
        if tool is None:
            raise RuntimeError("search_medical_codes not found in SearchServicesMCPToolClient")
        self.tool = tool
        ...
        try:
            ...
            if getattr(_s, "LANGFUSE_PROMPT_VERSIONING_ENABLED", False):
                ...
                _prompt = _tracer.get_prompt_or_default(
                    "csip-search-medical-codes-worker-prompt", WORKER_PROMPT, label=_label
                )
        except Exception:
    ...
    def execute(self, query: str, user_id: str, user_role: str, session_id: str, execution_id: str = "") -> Dict[str, Any]:
        ...
        while retry_count <= max_retries:
            try:
                ...
                # Audit action — keyed by query + code_type, no single resource_id
                ...
                # (e.g. procedures, diagnoses) that may be redacted from the
                # scrubbed output.
                _raw_tool_output = ""
