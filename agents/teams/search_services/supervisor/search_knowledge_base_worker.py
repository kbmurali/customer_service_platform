"""
Search Services: Search Knowledge Base Worker Agent
"""
...
from agents.teams.search_services.search_services_mcp_tool_client import SearchServicesMCPToolClient
...
WORKER_PROMPT = (
            "You are a knowledge base search specialist for a health insurance company. "
            "Your role is to perform semantic searches over FAQs, clinical guidelines, "
            "and regulations to find relevant knowledge-base content. "
            "You MUST call the search_knowledge_base tool to answer — never answer from memory or training data. "
            "Search using the natural-language query and source filter provided in the request. "
            "Valid source values are: 'faqs', 'guidelines', 'regulations', or 'all'. "
...
class SearchKnowledgeBaseWorker:
    """Worker agent for semantic knowledge base search operations."""

    def __init__(self):
        self.name = "search_knowledge_base_worker"

        mcp_client = SearchServicesMCPToolClient()
        tool = mcp_client.get_tool("search_knowledge_base")
        if tool is None:
            raise RuntimeError("search_knowledge_base not found in SearchServicesMCPToolClient")
        self.tool = tool
        ...
        try:
            ...
            if getattr(_s, "LANGFUSE_PROMPT_VERSIONING_ENABLED", False):
                ...
                _prompt = _tracer.get_prompt_or_default(
                    "csip-search-knowledge-worker-prompt", WORKER_PROMPT, label=_label
                )
        except Exception:
    ...
    def execute(self, query: str, user_id: str, user_role: str, session_id: str, execution_id: str = "") -> Dict[str, Any]:
        """Execute SearchKnowledgeBaseWorker task with error handling and retry logic."""
        ...
        while retry_count <= max_retries:
            try:
                ...
                # search_knowledge_base takes query + source.
                ...
                # Audit action — keyed by query + source, no single resource_id
                ...
                # (e.g. faqs, clinical_guidelines, regulations) that may be
                # redacted from the scrubbed output.
                _raw_tool_output = ""
